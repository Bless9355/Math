...
the reference line. Select any grid that is adjacent and below 
...
\new{Definiton} \textit{Fibbonacci inversion function}

Let 
\[
    \mathcal{F} (N) = n   
\]
where $n$ is the index of the largest fibbonacci number less than 
or equal to  $N$. 
...
Suppose $n - 1 = \mathcal{F} (m/2)$. In a 1D strip of the Conway game 
with multiplicity $m$, it is possible to rally $m + F_{n + 1}$ 
Soldiers to one target grid.
\proof Here is a visual demonstration. 
